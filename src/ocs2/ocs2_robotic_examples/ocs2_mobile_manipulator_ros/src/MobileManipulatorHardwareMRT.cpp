...
class MobileManipulatorHardwareMrtNode {
 public:
  ...
  void run() {
    ...
    const auto initTargetTrajectories = buildCurrentPoseTargetTrajectories(getLatestObservationOrThrow());
    ...
    while (rclcpp::ok()) {
      ...
      const auto desiredArmState = vector_t(getArmJointAngles(nextStateConst, modelInfo_));
